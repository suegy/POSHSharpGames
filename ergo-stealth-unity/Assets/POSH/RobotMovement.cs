	...
	public class RobotMovement : POSHBehaviour
	{
        ...
        void Update()
        {
            ...
            if (ergos is Dictionary<string, RampActivation> && ergos.Count > 1 && Time.realtimeSinceStartup - tickTimer > 0.3f)
            {
                ...
                if (!alarm.alarmOn && personalLastSighting == lastPlayerSighting.resetPosition && Time.realtimeSinceStartup - previousSightingTime > chaseWaitTime)
                {
                        ergos["chasing"].Reset();
            ...
            if (angle < fieldOfViewAngle * 0.5f)
            {
                ...
                if (Physics.Raycast(transform.position + transform.up, direction.normalized, out hit, col.radius ))
                {
                    ...
                    if (hit.collider.gameObject == player)
                    {
                        ...
                        playerInSight = true;
                        DebugOnScreen("playerInSight " + playerInSight);
        ...
        internal void Shooting()
        {
            ...
            {
                nav.Stop();
                AnimateShooting(playerInSight);
            });
            
        }

        internal void AnimateShooting(bool inSight)
        {
            DebugOnScreen("animateshooting "+inSight);
            ...
            {
                DebugOnScreen("try shooting ");
                // ... set the animator parameter to whether the player is in sight or not.
                //if (anim.GetBool(hash.playerInSightBool) != inSight)
                //{
                    DebugOnScreen("change shooting state "+inSight);
                    anim.SetBool(hash.playerInSightBool, inSight);
        ...
        internal void Recharging()
        {
            ...
            Loom.QueueOnMainThread(() =>
            {
                ...
                if (nav.remainingDistance < nav.stoppingDistance && nextToCharger)
                {
                    nav.Stop();
                    // ... increment the timer.
        ...
        private void DebugOnScreen(string message)
        {
            Loom.QueueOnMainThread(() =>
               {  
                    if (IndexInString(debugText.text,"\n",10) > 0)
                    {
                        int idx = IndexInString(debugText.text, "\n", 5);
                        debugText.text = debugText.text.Substring(idx);
                    }
                    debugText.text += id+ ": " + message + "\n";
               });
        }
